...
/**
 * POST /api/correction/export-dxf
 * 補正済みBlueprintJsonからDXFファイルを生成してダウンロード
 *
 * Body: BlueprintJson (補正済み)
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const tmpDir = join(process.cwd(), 'output', 'correction');
    ...
    await new Promise<void>((resolve, reject) => {
      exec(
        `python "${scriptPath}" --json "${jsonPath}" -o "${dxfPath}"`,
    ...
    const projectName = body.projectName || body.project_name || 'corrected_plan';
