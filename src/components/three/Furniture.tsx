...
const MATERIAL_PBR: Record<FurnitureMaterial, FurniturePBR> = {
  ...
  metal:   { roughness: 0.15, metalness: 0.8  },
  ...
  stone:   { roughness: 0.8,  metalness: 0.0  },
};
...
function Counter({ scale, color, palette, pbr, selected }: FurniturePartProps) {
  ...
  return (
    <group>
      {/* 天板（オーバーハング付き） */}
      <RoundedBox args={[w + 0.03, 0.05, d + 0.02]} radius={0.01} position={[0, h, 0]} castShadow receiveShadow>
...
function TableSquare({ scale, color, palette, pbr, selected }: FurniturePartProps) {
  ...
  return (
    <group>
      {/* 天板 */}
      <RoundedBox args={[w, 0.04, d]} radius={0.015} position={[0, h, 0]} castShadow receiveShadow>
      ...
      {/* 天板エッジハイライト */}
      <mesh position={[0, h + 0.021, 0]}>
        <boxGeometry args={[w - 0.01, 0.002, d - 0.01]} />
        <meshPhysicalMaterial color={adjustColor(topColor, 15)} roughness={0.3} metalness={pbr.metalness} clearcoat={0.1} clearcoatRoughness={0.5} />
      </mesh>
      {/* 幕板（天板下の水平ビーム） */}
      {/* 前後 */}
      {[-1, 1].map((z, i) => (
        <mesh key={`apron-z-${i}`} position={[0, h - 0.04 - apronH / 2, z * (d / 2 - legInset)]}>
          <boxGeometry args={[w - legInset * 2, apronH, 0.015]} />
          <meshPhysicalMaterial color={color ? adjustColor(color, -15) : palette.primary} roughness={pbr.roughness} metalness={pbr.metalness} clearcoat={0.1} clearcoatRoughness={0.5} />
        </mesh>
      ))}
      {/* 左右 */}
      {[-1, 1].map((x, i) => (
        <mesh key={`apron-x-${i}`} position={[x * (w / 2 - legInset), h - 0.04 - apronH / 2, 0]}>
      ...
      {[[-1, -1], [1, -1], [-1, 1], [1, 1]].map(([x, z], i) => (
        <group key={i} position={[x * (w / 2 - legInset), 0, z * (d / 2 - legInset)]}>
          ...
          <mesh position={[0, (h - 0.04) / 2, 0]} castShadow>
            <cylinderGeometry args={[legTopR, legBotR, h - 0.04, 8]} />
          ...
          <mesh position={[0, 0.0025, 0]}>
            <cylinderGeometry args={[footR, footR, 0.005, 12]} />
            <meshPhysicalMaterial color={color ? adjustColor(color, -40) : adjustColor(palette.metal, -10)} roughness={0.4} metalness={0.1} clearcoat={0.3} clearcoatRoughness={0.2} />
          </mesh>
        </group>
      ))}
    </group>
...
function Chair({ scale, color, palette, pbr, selected }: FurniturePartProps) {
  ...
  return (
    <group>
      ...
      {/* 座面クッション */}
      <RoundedBox
        args={[w - 0.03, 0.03, d - 0.03]}
        radius={0.015}
        position={[0, seatH + 0.028, 0]}
        rotation={[-0.03, 0, 0]}
        castShadow
      >
        <meshPhysicalMaterial color={cushionColor} roughness={pbr.roughness * 1.2} metalness={0} clearcoat={0} clearcoatRoughness={0.4} emissive={cushionColor} emissiveIntensity={selected ? 0.15 : 0} />
      </RoundedBox>
      {/* 背もたれフレーム */}
      <RoundedBox
      ...
      {[0.62, 0.82].map((ratio, i) => (
        <mesh key={`slat-${i}`} position={[0, h * ratio, -d / 2 + 0.028]} rotation={[0.08, 0, 0]}>
          <boxGeometry args={[w - 0.06, 0.012, 0.008]} />
      ...
      {[[-1, -1], [1, -1], [-1, 1], [1, 1]].map(([x, z], i) => (
        <mesh
          ...
          castShadow
        >
          <cylinderGeometry args={[legR, legBotR, seatH, 8]} />
          <meshPhysicalMaterial color={legColor} roughness={pbr.roughness} metalness={pbr.metalness} clearcoat={0.3} clearcoatRoughness={0.2} />
        </mesh>
      ))}
      {/* 前脚クロスバー */}
      <mesh position={[0, 0.15, d / 2 - legInset]}>
        <boxGeometry args={[w - legInset * 2, 0.012, 0.012]} />
        <meshPhysicalMaterial color={legColor} roughness={pbr.roughness} metalness={pbr.metalness} clearcoat={0.3} clearcoatRoughness={0.2} />
      </mesh>
    </group>
  );
...
function Sofa({ scale, color, palette, pbr, selected }: FurniturePartProps) {
  ...
  return (
    <group>
      ...
      {/* 脚 — 小さな装飾パーツのためcastShadow省略 */}
      {[[-1, -1], [1, -1], [-1, 1], [1, 1]].map(([x, z], i) => (
        <mesh key={`foot-${i}`} position={[x * (w / 2 - 0.06), footR, z * (d / 2 - 0.06)]}>
          <cylinderGeometry args={[footR, footR, footR * 2, 8]} />
          <meshPhysicalMaterial color={legColor} roughness={0.4} metalness={0.1} clearcoat={0.3} clearcoatRoughness={0.2} />
        </mesh>
      ))}
      {/* 座面クッション（2分割） */}
      {[-1, 1].map((side, i) => (
        <RoundedBox
          ...
          castShadow
        >
          <meshPhysicalMaterial color={cushionColor} roughness={0.95} metalness={0} clearcoat={0} clearcoatRoughness={0.4} emissive={cushionColor} emissiveIntensity={selected ? 0.15 : 0} />
        </RoundedBox>
      ))}
      {/* 背もたれクッション（2分割） */}
      {[-1, 1].map((side, i) => (
        <RoundedBox
          ...
          castShadow
        >
          <meshPhysicalMaterial color={color ? adjustColor(color, -8) : adjustColor(palette.fabric, -8)} roughness={0.95} metalness={0} clearcoat={0} clearcoatRoughness={0.4} emissive={color ? adjustColor(color, -8) : adjustColor(palette.fabric, -8)} emissiveIntensity={selected ? 0.15 : 0} />
        </RoundedBox>
      ))}
      {/* 背もたれフレーム */}
      <RoundedBox
...
function PendantLight({ scale, color, palette }: FurniturePartProps) {
  ...
  const shadeGeo = new THREE.LatheGeometry(shadePoints, 24);
  // 内側シェード用（少し小さく）
  ...
  const innerGeo = new THREE.LatheGeometry(innerPoints, 24);
  // シェード底面のY座標
  ...
  return (
    <group>
      ...
      <mesh position={[0, h, 0]}>
        ...
        <meshStandardMaterial color={color ? '#444' : palette.metal} metalness={0.5} roughness={0.3} envMapIntensity={1.5} />
      </mesh>
      {/* コード — palette.metal連動 */}
      <mesh position={[0, h / 2, 0]}>
...
function Plant({ scale, color, palette }: FurniturePartProps) {
  const [w, h] = scale;
  const style = useEditorStore((s) => s.style);
  ...
  const potGeo = new THREE.LatheGeometry(potPoints, 20);
  // 葉の配置（放射状、角度・傾きを多様に）
  ...
  return (
    <group>
      ...
      <mesh geometry={potGeo} castShadow>
        <meshStandardMaterial color={potColor} roughness={potRoughness} metalness={potMetalness} />
      </mesh>
      {/* 土（鉢リム内側に少し凹ませる） */}
...
function DisplayCase({ scale, color, palette: _palette }: FurniturePartProps) {
  const [w, h, d] = scale;
  const baseH = 0.08;
    ...
    <group>
      ...
      {[[-1, -1], [1, -1], [-1, 1], [1, 1]].map(([x, z], i) => (
        <mesh key={`foot-${i}`} position={[x * (w / 2 - 0.03), footH / 2, z * (d / 2 - 0.03)]}>
          <cylinderGeometry args={[footR, footR, footH, 8]} />
      ...
      {/* ガラス棚板（2枚） */}
      {[1, 2].map((i) => {
        const shelfY = baseH + footH + ((h - baseH - topH - footH) / 3) * i;
        return (
