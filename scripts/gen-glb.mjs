...
const furniture = {
  chair: () => {
    // Mid-Century "Cow Horn" Chair — Black Ebony + Woven Cane Seat
    // Total height: 750mm, seat height: 450mm
    const BE = [0.04, 0.035, 0.03];   // Black Ebony (matte dark)
    const CN = [0.82, 0.72, 0.50];    // Woven Cane (natural honey)
    const CND = [0.65, 0.55, 0.35];   // Cane shadow/weave detail

    const parts = [
      // === 座面フレーム (円形に近いD型、フレーム溝付き) ===
      // フレーム外枠リング — 前方やや広め
      { ...merge([
        cyl(0.22, 0.22, 0.025, 20, 0, 0.45, 0.01),  // 座面リング
      ]), color: BE, name: 'seat_ring', roughness: 0.38 },
      // 座面フレーム上面（溝縁）
      { ...merge([
        cyl(0.215, 0.215, 0.008, 20, 0, 0.465, 0.01),
      ]), color: BE, name: 'seat_ring_top', roughness: 0.35 },

      // === ウーブンケイン座面 ===
      // メイン座面（薄いディスク — 織り目のある平面）
      { ...merge([
        cyl(0.20, 0.20, 0.012, 20, 0, 0.455, 0.01),
      ]), color: CN, name: 'cane_seat', roughness: 0.75 },
      // ケイン織目格子パターン（十字 + 対角線）
      { ...merge([
        box(0.36, 0.014, 0.004, 0, 0.456, 0.01),      // 横線
        box(0.004, 0.014, 0.36, 0, 0.456, 0.01),      // 縦線
        box(0.004, 0.014, 0.36, -0.06, 0.456, 0.01),  // 縦線2
        box(0.004, 0.014, 0.36, 0.06, 0.456, 0.01),   // 縦線3
        box(0.36, 0.014, 0.004, 0, 0.456, -0.05),     // 横線2
        box(0.36, 0.014, 0.004, 0, 0.456, 0.07),      // 横線3
      ]), color: CND, name: 'cane_weave', roughness: 0.80 },

      // === カウホーン背もたれ＋アームレスト一体型 ===
      // 背もたれ中央パッド（楕円形、薄い）
      { ...merge([
        box(0.16, 0.10, 0.018, 0, 0.66, -0.195),
      ]), color: BE, name: 'backrest_pad', roughness: 0.38 },
      // 背もたれ上部レール（カーブの頂点）
      { ...merge([
        cyl(0.014, 0.014, 0.18, 14, 0, 0.72, -0.195),
      ]), color: BE, name: 'backrest_top', roughness: 0.35 },
      // 背もたれからアームレストへの曲線 — 左ホーン
      // (複数の短い円柱セグメントでカーブを近似)
      { ...merge([
        cyl(0.013, 0.014, 0.08, 12, -0.11, 0.71, -0.185),  // 付け根
        cyl(0.012, 0.013, 0.08, 12, -0.17, 0.69, -0.160),  // カーブ1
        cyl(0.011, 0.012, 0.08, 12, -0.22, 0.66, -0.120),  // カーブ2
        cyl(0.010, 0.011, 0.08, 12, -0.25, 0.62, -0.065),  // カーブ3
        cyl(0.009, 0.010, 0.08, 12, -0.26, 0.57, -0.005),  // カーブ4
        cyl(0.008, 0.009, 0.08, 12, -0.25, 0.52, 0.050),   // 先端
      ]), color: BE, name: 'horn_left', roughness: 0.35 },
      // 右ホーン（左の鏡像）
      { ...merge([
        cyl(0.013, 0.014, 0.08, 12, 0.11, 0.71, -0.185),
        cyl(0.012, 0.013, 0.08, 12, 0.17, 0.69, -0.160),
        cyl(0.011, 0.012, 0.08, 12, 0.22, 0.66, -0.120),
        cyl(0.010, 0.011, 0.08, 12, 0.25, 0.62, -0.065),
        cyl(0.009, 0.010, 0.08, 12, 0.26, 0.57, -0.005),
        cyl(0.008, 0.009, 0.08, 12, 0.25, 0.52, 0.050),
      ]), color: BE, name: 'horn_right', roughness: 0.35 },
      // ホーン先端キャップ（丸い端）
      { ...merge([
        cyl(0.010, 0.002, 0.015, 10, -0.245, 0.49, 0.075),
        cyl(0.010, 0.002, 0.015, 10, 0.245, 0.49, 0.075),
      ]), color: BE, name: 'horn_tips', roughness: 0.30 },

      // === 背もたれ支柱（座面フレームから背もたれへの接続） ===
      { ...merge([
        cyl(0.012, 0.016, 0.26, 14, -0.10, 0.58, -0.18),  // 左支柱
        cyl(0.012, 0.016, 0.26, 14, 0.10, 0.58, -0.18),   // 右支柱
      ]), color: BE, name: 'back_struts', roughness: 0.35 },

      // === テーパー脚4本 ===
      // 前脚（テーパー比 1:1.5, やや前方に開く）
      { ...merge([
        cyl(0.010, 0.018, 0.44, 16, -0.17, 0.22, 0.14),
      ]), color: BE, name: 'leg_fl', roughness: 0.38 },
      { ...merge([
        cyl(0.010, 0.018, 0.44, 16, 0.17, 0.22, 0.14),
      ]), color: BE, name: 'leg_fr', roughness: 0.38 },
      // 後脚（スプレイ — やや外側に開く、安定性のため）
      { ...merge([
        cyl(0.010, 0.018, 0.44, 16, -0.16, 0.22, -0.16),
      ]), color: BE, name: 'leg_bl', roughness: 0.38 },
      { ...merge([
        cyl(0.010, 0.018, 0.44, 16, 0.16, 0.22, -0.16),
      ]), color: BE, name: 'leg_br', roughness: 0.38 },

      // 脚先端フェルト
      { ...merge([
        cyl(0.012, 0.012, 0.004, 8, -0.17, 0.002, 0.14),
        cyl(0.012, 0.012, 0.004, 8, 0.17, 0.002, 0.14),
        cyl(0.012, 0.012, 0.004, 8, -0.16, 0.002, -0.16),
        cyl(0.012, 0.012, 0.004, 8, 0.16, 0.002, -0.16),
      ]), color: [0.25, 0.25, 0.25], name: 'leg_pads', roughness: 0.9 },

      // === 座面-脚接合部 ===
      { ...merge([
        cyl(0.018, 0.018, 0.020, 12, -0.17, 0.44, 0.14),
        cyl(0.018, 0.018, 0.020, 12, 0.17, 0.44, 0.14),
        cyl(0.018, 0.018, 0.020, 12, -0.16, 0.44, -0.16),
        cyl(0.018, 0.018, 0.020, 12, 0.16, 0.44, -0.16),
      ]), color: BE, name: 'leg_joints', roughness: 0.35 },

      // === 横桟（側面のみ — ミニマルデザイン） ===
      { ...merge([
        cyl(0.007, 0.007, 0.28, 8, -0.165, 0.15, -0.01),
        cyl(0.007, 0.007, 0.28, 8, 0.165, 0.15, -0.01),
      ]), color: BE, name: 'stretchers', roughness: 0.38 },
    ];
    return parts.map(p => ({ ...merge([p]), color: p.color, name: p.name, roughness: p.roughness }));
  },
  table_square: () => [
    // 天板本体
    { ...merge([box(0.88, 0.032, 0.88, 0, 0.74, 0)]), color: WL, name: 'top' },
    // 天板面取りエッジ（4辺 + 上面エッジ）
    { ...merge([box(0.90, 0.008, 0.90, 0, 0.722, 0)]), color: WL, name: 'top_bevel_bottom' },
    { ...merge([cyl(0.006, 0.006, 0.88, 8, 0, 0.757, 0.44), cyl(0.006, 0.006, 0.88, 8, 0, 0.757, -0.44)]), color: WL, name: 'top_bevel_fb' },
    { ...merge([cyl(0.006, 0.006, 0.88, 8, 0.44, 0.757, 0), cyl(0.006, 0.006, 0.88, 8, -0.44, 0.757, 0)]), color: WL, name: 'top_bevel_lr' },
    // 天板の木目方向アクセント（2本の薄いライン）
    { ...merge([box(0.84, 0.003, 0.004, 0, 0.758, -0.20), box(0.84, 0.003, 0.004, 0, 0.758, 0.20)]), color: [0.70, 0.55, 0.35], name: 'wood_grain' },
    // テーパー脚（分割数16で滑らか）
    { ...merge([cyl(0.020, 0.030, 0.70, 14, -0.39, 0.35, -0.39)]), color: WD, name: 'leg_bl' },
    { ...merge([cyl(0.020, 0.030, 0.70, 14, 0.39, 0.35, -0.39)]), color: WD, name: 'leg_br' },
    { ...merge([cyl(0.020, 0.030, 0.70, 14, -0.39, 0.35, 0.39)]), color: WD, name: 'leg_fl' },
    { ...merge([cyl(0.020, 0.030, 0.70, 14, 0.39, 0.35, 0.39)]), color: WD, name: 'leg_fr' },
    // 脚上部接合リング
    { ...merge([cyl(0.026, 0.026, 0.012, 12, -0.39, 0.70, -0.39), cyl(0.026, 0.026, 0.012, 12, 0.39, 0.70, -0.39), cyl(0.026, 0.026, 0.012, 12, -0.39, 0.70, 0.39), cyl(0.026, 0.026, 0.012, 12, 0.39, 0.70, 0.39)]), color: WD, name: 'leg_collars' },
    // 幕板（4面、少し丸みを持たせた厚さ）
    { ...merge([box(0.76, 0.050, 0.022, 0, 0.695, 0.41), box(0.76, 0.050, 0.022, 0, 0.695, -0.41)]), color: W, name: 'apron_fb' },
    { ...merge([box(0.022, 0.050, 0.80, 0.41, 0.695, 0), box(0.022, 0.050, 0.80, -0.41, 0.695, 0)]), color: W, name: 'apron_lr' },
    // 幕板下端のモールディング
    { ...merge([box(0.76, 0.008, 0.026, 0, 0.665, 0.41), box(0.76, 0.008, 0.026, 0, 0.665, -0.41)]), color: WD, name: 'apron_molding_fb' },
    { ...merge([box(0.026, 0.008, 0.80, 0.41, 0.665, 0), box(0.026, 0.008, 0.80, -0.41, 0.665, 0)]), color: WD, name: 'apron_molding_lr' },
    // 横桟（H型補強、円柱で丸桟）
    { ...merge([cyl(0.012, 0.012, 0.74, 8, 0, 0.15, 0)]), color: WD, name: 'stretcher_x' },
    { ...merge([cyl(0.012, 0.012, 0.74, 8, 0, 0.15, 0)]), color: WD, name: 'stretcher_z' },
    // 脚底フェルト
    { ...merge([cyl(0.028, 0.028, 0.005, 8, -0.39, 0.002, -0.39), cyl(0.028, 0.028, 0.005, 8, 0.39, 0.002, -0.39), cyl(0.028, 0.028, 0.005, 8, -0.39, 0.002, 0.39), cyl(0.028, 0.028, 0.005, 8, 0.39, 0.002, 0.39)]), color: [0.3, 0.3, 0.3], name: 'felt_pads' },
  ],
  sofa: () => [
    // フレーム（木製ベース）
    { ...merge([box(1.60, 0.10, 0.75, 0, 0.15, 0)]), color: WD, name: 'frame' },
    // 脚（テーパー円柱4本、分割数増）
    { ...merge([cyl(0.020, 0.028, 0.08, 12, -0.70, 0.04, -0.30), cyl(0.020, 0.028, 0.08, 12, 0.70, 0.04, -0.30), cyl(0.020, 0.028, 0.08, 12, -0.70, 0.04, 0.30), cyl(0.020, 0.028, 0.08, 12, 0.70, 0.04, 0.30)]), color: WD, name: 'legs' },
    // 座面クッション3分割（膨らみ2層表現）
    { ...merge([box(0.46, 0.11, 0.58, -0.50, 0.32, 0.04)]), color: FR, name: 'cushion_l_base', roughness: 0.85 },
    { ...merge([box(0.42, 0.03, 0.54, -0.50, 0.39, 0.04)]), color: FR, name: 'cushion_l_top', roughness: 0.90 },
    { ...merge([box(0.46, 0.11, 0.58, 0, 0.32, 0.04)]), color: FR, name: 'cushion_c_base', roughness: 0.85 },
    { ...merge([box(0.42, 0.03, 0.54, 0, 0.39, 0.04)]), color: FR, name: 'cushion_c_top', roughness: 0.90 },
    { ...merge([box(0.46, 0.11, 0.58, 0.50, 0.32, 0.04)]), color: FR, name: 'cushion_r_base', roughness: 0.85 },
    { ...merge([box(0.42, 0.03, 0.54, 0.50, 0.39, 0.04)]), color: FR, name: 'cushion_r_top', roughness: 0.90 },
    // クッション分割線（深い溝）
    { ...merge([box(0.008, 0.14, 0.56, -0.25, 0.33, 0.04), box(0.008, 0.14, 0.56, 0.25, 0.33, 0.04)]), color: [0.60, 0.20, 0.20], name: 'cushion_dividers' },
    // 背もたれクッション3分割（膨らみ表現）
    { ...merge([box(0.46, 0.34, 0.10, -0.50, 0.58, -0.28)]), color: FR, name: 'back_l', roughness: 0.85 },
    { ...merge([box(0.42, 0.30, 0.03, -0.50, 0.58, -0.22)]), color: FR, name: 'back_l_puff', roughness: 0.90 },
    { ...merge([box(0.46, 0.36, 0.10, 0, 0.59, -0.28)]), color: FR, name: 'back_c', roughness: 0.85 },
    { ...merge([box(0.42, 0.32, 0.03, 0, 0.59, -0.22)]), color: FR, name: 'back_c_puff', roughness: 0.90 },
    { ...merge([box(0.46, 0.34, 0.10, 0.50, 0.58, -0.28)]), color: FR, name: 'back_r', roughness: 0.85 },
    { ...merge([box(0.42, 0.30, 0.03, 0.50, 0.58, -0.22)]), color: FR, name: 'back_r_puff', roughness: 0.90 },
    // 背もたれ分割線
    { ...merge([box(0.008, 0.35, 0.12, -0.25, 0.58, -0.28), box(0.008, 0.35, 0.12, 0.25, 0.58, -0.28)]), color: [0.60, 0.20, 0.20], name: 'back_dividers' },
    // アームレスト（丸み: 前面+上面を円柱で）
    { ...merge([box(0.10, 0.20, 0.56, -0.76, 0.40, 0.02)]), color: FR, name: 'arm_l', roughness: 0.85 },
    { ...merge([cyl(0.05, 0.05, 0.56, 12, -0.76, 0.51, 0.02)]), color: FR, name: 'arm_l_top', roughness: 0.85 },
    { ...merge([cyl(0.05, 0.10, 0.10, 12, -0.76, 0.40, 0.32)]), color: FR, name: 'arm_l_front', roughness: 0.85 },
    { ...merge([box(0.10, 0.20, 0.56, 0.76, 0.40, 0.02)]), color: FR, name: 'arm_r', roughness: 0.85 },
    { ...merge([cyl(0.05, 0.05, 0.56, 12, 0.76, 0.51, 0.02)]), color: FR, name: 'arm_r_top', roughness: 0.85 },
    { ...merge([cyl(0.05, 0.10, 0.10, 12, 0.76, 0.40, 0.32)]), color: FR, name: 'arm_r_front', roughness: 0.85 },
    // クッションピロー（ふっくら2層）
    { ...merge([box(0.18, 0.09, 0.08, -0.55, 0.49, -0.15)]), color: FB, name: 'pillow_l', roughness: 0.9 },
    { ...merge([box(0.15, 0.04, 0.06, -0.55, 0.55, -0.15)]), color: FB, name: 'pillow_l_top', roughness: 0.9 },
    { ...merge([box(0.18, 0.09, 0.08, 0.55, 0.49, -0.15)]), color: FB, name: 'pillow_r', roughness: 0.9 },
    { ...merge([box(0.15, 0.04, 0.06, 0.55, 0.55, -0.15)]), color: FB, name: 'pillow_r_top', roughness: 0.9 },
  ],
  counter: () => [
    // 天板（オーバーハング + 厚み感）
    { ...merge([box(1.54, 0.038, 0.64, 0, 0.935, 0)]), color: CR, name: 'top' },
    // 天板エッジ（面取り表現）
    { ...merge([box(1.56, 0.010, 0.66, 0, 0.912, 0)]), color: CR, name: 'top_bevel' },
    // 天板前面ノーズ（丸みエッジ）
    { ...merge([cyl(0.008, 0.008, 1.54, 10, 0, 0.955, 0.32)]), color: CR, name: 'top_nose' },
    // 前面パネル
    { ...merge([box(1.46, 0.58, 0.028, 0, 0.62, 0.29)]), color: W, name: 'front_panel' },
    // 前面パネル装飾モールディング（上下ライン）
    { ...merge([box(1.42, 0.010, 0.015, 0, 0.90, 0.305)]), color: WD, name: 'front_molding_top' },
    { ...merge([box(1.42, 0.010, 0.015, 0, 0.32, 0.305)]), color: WD, name: 'front_molding_bot' },
    // 前面パネル中央仕切り
    { ...merge([box(0.012, 0.56, 0.010, 0, 0.62, 0.305)]), color: WD, name: 'front_divider' },
    // 側面パネル
    { ...merge([box(0.028, 0.58, 0.56, -0.73, 0.62, 0), box(0.028, 0.58, 0.56, 0.73, 0.62, 0)]), color: W, name: 'side_panels' },
    // 背面パネル
    { ...merge([box(1.46, 0.58, 0.018, 0, 0.62, -0.29)]), color: WD, name: 'back_panel' },
    // 棚板2段（エッジ付き）
    { ...merge([box(1.40, 0.018, 0.52, 0, 0.50, 0)]), color: WL, name: 'shelf_upper' },
    { ...merge([box(1.40, 0.008, 0.015, 0, 0.50, 0.27)]), color: WD, name: 'shelf_upper_edge' },
    { ...merge([box(1.40, 0.018, 0.52, 0, 0.35, 0)]), color: WL, name: 'shelf_lower' },
    { ...merge([box(1.40, 0.008, 0.015, 0, 0.35, 0.27)]), color: WD, name: 'shelf_lower_edge' },
    // 内部仕切り板
    { ...merge([box(0.015, 0.28, 0.50, -0.35, 0.44, 0), box(0.015, 0.28, 0.50, 0.35, 0.44, 0)]), color: WL, name: 'dividers' },
    // 幕板（前面装飾ライン）
    { ...merge([box(1.42, 0.025, 0.008, 0, 0.88, 0.30)]), color: WD, name: 'apron' },
    // 脚（短い台座 + アジャスター）
    { ...merge([box(0.05, 0.035, 0.05, -0.68, 0.018, -0.24), box(0.05, 0.035, 0.05, 0.68, 0.018, -0.24), box(0.05, 0.035, 0.05, -0.68, 0.018, 0.24), box(0.05, 0.035, 0.05, 0.68, 0.018, 0.24)]), color: MD, name: 'feet' },
    { ...merge([cyl(0.015, 0.015, 0.012, 8, -0.68, 0.002, -0.24), cyl(0.015, 0.015, 0.012, 8, 0.68, 0.002, -0.24), cyl(0.015, 0.015, 0.012, 8, -0.68, 0.002, 0.24), cyl(0.015, 0.015, 0.012, 8, 0.68, 0.002, 0.24)]), color: [0.3, 0.3, 0.3], name: 'adjusters' },
  ],
  stool: () => [
    // 座面ベース
  ...
  bookcase: () => [
    ...
    { ...merge([box(0.93, 0.012, 0.37, 0, 1.83, 0)]), color: WD, name: 'crown_molding' },
    // 背板
    ...
    // 本の背表紙（段3〜5に、ランダム幅・高さの「本」をboxで表現）
    // 段5（1.46〜1.82）: 高めの本
    { ...merge([box(0.05, 0.32, 0.22, -0.30, 1.63, 0.00), box(0.07, 0.30, 0.22, -0.20, 1.62, 0.00), box(0.04, 0.34, 0.22, -0.10, 1.64, 0.00), box(0.06, 0.28, 0.22, -0.02, 1.61, 0.00), box(0.05, 0.32, 0.22, 0.06, 1.63, 0.00), box(0.07, 0.26, 0.22, 0.15, 1.60, 0.00), box(0.04, 0.30, 0.22, 0.24, 1.62, 0.00), box(0.06, 0.33, 0.22, 0.32, 1.635, 0.00)]), color: [0.18, 0.31, 0.56], name: 'books_top' },
    // 段4（1.10〜1.46）
    { ...merge([box(0.06, 0.30, 0.20, -0.28, 1.26, 0.00), box(0.05, 0.34, 0.20, -0.18, 1.28, 0.00), box(0.07, 0.28, 0.20, -0.08, 1.25, 0.00), box(0.04, 0.32, 0.20, 0.02, 1.27, 0.00), box(0.06, 0.30, 0.20, 0.12, 1.26, 0.00), box(0.05, 0.26, 0.20, 0.22, 1.24, 0.00)]), color: [0.56, 0.18, 0.22], name: 'books_mid_upper' },
    // 段3（0.74〜1.10）
    { ...merge([box(0.07, 0.28, 0.20, -0.25, 0.89, 0.00), box(0.05, 0.34, 0.20, -0.14, 0.92, 0.00), box(0.06, 0.30, 0.20, -0.04, 0.90, 0.00), box(0.04, 0.32, 0.20, 0.06, 0.91, 0.00), box(0.07, 0.26, 0.20, 0.16, 0.88, 0.00), box(0.05, 0.34, 0.20, 0.26, 0.92, 0.00), box(0.06, 0.28, 0.20, 0.35, 0.89, 0.00)]), color: [0.22, 0.45, 0.28], name: 'books_mid_lower' },
    // 下段扉（キャビネット部分: パネル2枚）
    { ...merge([box(0.42, 0.34, 0.015, -0.22, 0.20, 0.18)]), color: W, name: 'door_l' },
    { ...merge([box(0.42, 0.34, 0.015, 0.22, 0.20, 0.18)]), color: W, name: 'door_r' },
    // 扉パネル装飾（凹み表現）
    { ...merge([box(0.34, 0.26, 0.005, -0.22, 0.20, 0.19), box(0.34, 0.26, 0.005, 0.22, 0.20, 0.19)]), color: [0.90, 0.87, 0.78], name: 'door_panels' },
    // 扉取っ手（丸型ノブ）
    { ...merge([cyl(0.012, 0.012, 0.02, 10, -0.03, 0.20, 0.20), cyl(0.012, 0.012, 0.02, 10, 0.03, 0.20, 0.20)]), color: MS, name: 'door_handles', metallic: 0.7 },
    // 台座（巾木）
    { ...merge([box(0.90, 0.04, 0.02, 0, 0.02, 0.18)]), color: WD, name: 'plinth' },
  ],
  // ─── Additional furniture types ───
  table_round: () => {
    // Modern Marble Cafe Table — Carrara marble top + matte black metal pedestal
    // Total height: 750mm, top diameter: 700mm, base diameter: 400mm
    const MB = [0.05, 0.05, 0.05];    // Matte Black Metal
    const MR = [0.92, 0.90, 0.88];    // Carrara Marble (white-grey)
    const MRV = [0.75, 0.74, 0.73];   // Marble vein accent
    return [
      // === 天板: カラーラ大理石 φ700mm ===
      // メイン天板ディスク（厚さ25mm）
      { ...merge([cyl(0.35, 0.35, 0.025, 24, 0, 0.738, 0)]), color: MR, name: 'top', roughness: 0.15, metallic: 0.0 },
      // 天板エッジ面取り（わずかなベベル）
      { ...merge([cyl(0.352, 0.352, 0.008, 24, 0, 0.722, 0)]), color: MR, name: 'top_bevel', roughness: 0.12 },
      // 天板裏面（少し暗め）
      { ...merge([cyl(0.34, 0.34, 0.006, 20, 0, 0.722, 0)]), color: MRV, name: 'top_underside', roughness: 0.25 },
      // 大理石脈模様（天板上面に薄い線として表現）
      { ...merge([
        box(0.50, 0.003, 0.008, -0.05, 0.752, 0.02),     // 主脈1
        box(0.35, 0.003, 0.006, 0.08, 0.752, -0.08),     // 主脈2
        box(0.25, 0.003, 0.005, -0.10, 0.752, 0.12),     // 副脈1
        box(0.18, 0.003, 0.004, 0.15, 0.752, -0.15),     // 副脈2
      ]), color: MRV, name: 'marble_veins', roughness: 0.20 },

      // === 支柱上部フランジ（天板マウント） ===
      { ...merge([cyl(0.05, 0.04, 0.020, 14, 0, 0.715, 0)]), color: MB, name: 'top_flange', roughness: 0.45, metallic: 0.85 },

      // === 支柱: マットブラックメタル φ40mm→φ35mm テーパー ===
      { ...merge([cyl(0.035, 0.040, 0.62, 14, 0, 0.40, 0)]), color: MB, name: 'pedestal', roughness: 0.45, metallic: 0.85 },

      // === 支柱下部カラー（ベースとの接合リング） ===
      { ...merge([cyl(0.045, 0.060, 0.030, 14, 0, 0.095, 0)]), color: MB, name: 'pedestal_collar', roughness: 0.45, metallic: 0.85 },

      // === 台座: トランペット型 φ400mm ===
      // ベース上面（テーパー遷移 — 支柱→台座）
      { ...merge([cyl(0.06, 0.16, 0.04, 16, 0, 0.06, 0)]), color: MB, name: 'base_taper', roughness: 0.45, metallic: 0.85 },
      // ベースディスク（フラット、φ400mm、厚さ12mm）
      { ...merge([cyl(0.20, 0.20, 0.012, 20, 0, 0.036, 0)]), color: MB, name: 'base_disc', roughness: 0.50, metallic: 0.85 },
      // ベースエッジ（わずかなリップ）
      { ...merge([cyl(0.205, 0.205, 0.005, 20, 0, 0.032, 0)]), color: MB, name: 'base_edge', roughness: 0.40, metallic: 0.9 },
      // ベース底面フェルトパッド（4点）
      { ...merge([
        cyl(0.015, 0.015, 0.003, 8, 0.14, 0.0015, 0),
        cyl(0.015, 0.015, 0.003, 8, -0.14, 0.0015, 0),
        cyl(0.015, 0.015, 0.003, 8, 0, 0.0015, 0.14),
        cyl(0.015, 0.015, 0.003, 8, 0, 0.0015, -0.14),
      ]), color: [0.25, 0.25, 0.25], name: 'felt_pads', roughness: 0.9 },
    ];
  },
  pendant_light: () => [
    // 天井取付ベース（シーリングカップ）
    { ...merge([cyl(0.04, 0.04, 0.025, 12, 0, 0.99, 0)]), color: MD, name: 'ceiling_cup', metallic: 0.7 },
    // コード（細い円柱）
    { ...merge([cyl(0.004, 0.004, 0.35, 4, 0, 0.81, 0)]), color: [0.15, 0.15, 0.15], name: 'cord' },
    // コード留め
    { ...merge([cyl(0.008, 0.008, 0.015, 8, 0, 0.975, 0)]), color: MD, name: 'cord_grip', metallic: 0.6 },
    // シェード上部（ソケット接続部）
    { ...merge([cyl(0.025, 0.030, 0.03, 12, 0, 0.65, 0)]), color: MD, name: 'socket_cover', metallic: 0.6 },
    // シェード本体（コニカル形状、多段で滑らかに）
    { ...merge([cyl(0.035, 0.10, 0.04, 14, 0, 0.62, 0)]), color: [0.9, 0.8, 0.3], name: 'shade_upper' },
    { ...merge([cyl(0.10, 0.16, 0.06, 14, 0, 0.57, 0)]), color: [0.9, 0.8, 0.3], name: 'shade_mid' },
    { ...merge([cyl(0.16, 0.18, 0.04, 14, 0, 0.53, 0)]), color: [0.88, 0.78, 0.28], name: 'shade_lower' },
    // シェード下端リム
    { ...merge([cyl(0.18, 0.18, 0.006, 14, 0, 0.508, 0)]), color: [0.82, 0.72, 0.25], name: 'shade_rim' },
    // シェード内側（光反射面）
    { ...merge([cyl(0.04, 0.17, 0.12, 12, 0, 0.57, 0)]), color: [0.95, 0.90, 0.70], name: 'shade_inner' },
    // 電球（小さい球で表現）
    { ...merge([cyl(0.02, 0.02, 0.04, 10, 0, 0.61, 0)]), color: [1.0, 0.95, 0.80], name: 'bulb' },
  ],
  partition: () => [
  ...
  sink: () => [
    // リム（天板、ステンレス）
    { ...merge([box(0.60, 0.035, 0.50, 0, 0.855, 0)]), color: MS, name: 'rim', metallic: 0.8 },
    // シンクボウル外枠
    { ...merge([box(0.50, 0.005, 0.40, 0, 0.835, 0)]), color: [0.85, 0.85, 0.90], name: 'basin_rim' },
    // シンクボウル内部（凹み表現: 段差で深さ表現）
    { ...merge([box(0.48, 0.04, 0.38, 0, 0.81, 0)]), color: [0.88, 0.88, 0.93], name: 'basin_outer', metallic: 0.7 },
    { ...merge([box(0.44, 0.06, 0.34, 0, 0.78, 0)]), color: [0.90, 0.90, 0.95], name: 'basin_inner', metallic: 0.7 },
    { ...merge([box(0.40, 0.04, 0.30, 0, 0.73, 0)]), color: [0.92, 0.92, 0.95], name: 'basin_bottom', metallic: 0.7 },
    // 排水口
    { ...merge([cyl(0.02, 0.02, 0.01, 10, 0, 0.71, 0)]), color: MD, name: 'drain', metallic: 0.8 },
    // 蛇口ベース
    { ...merge([cyl(0.025, 0.030, 0.04, 12, 0, 0.89, -0.18)]), color: MS, name: 'faucet_base', metallic: 0.85 },
    // 蛇口アーム（垂直）
    { ...merge([cyl(0.012, 0.012, 0.18, 10, 0, 0.98, -0.18)]), color: MS, name: 'faucet_stem', metallic: 0.85 },
    // 蛇口スパウト（水平、前方に曲がる）
    { ...merge([cyl(0.010, 0.010, 0.14, 8, 0, 1.07, -0.10)]), color: MS, name: 'faucet_spout', metallic: 0.85 },
    // 蛇口先端
    { ...merge([cyl(0.008, 0.012, 0.03, 8, 0, 1.05, -0.03)]), color: MS, name: 'faucet_tip', metallic: 0.85 },
    // レバーハンドル
    { ...merge([cyl(0.006, 0.006, 0.06, 6, 0.04, 1.00, -0.18), cyl(0.006, 0.006, 0.06, 6, -0.04, 1.00, -0.18)]), color: MS, name: 'faucet_handles', metallic: 0.85 },
    // キャビネット本体
    { ...merge([box(0.58, 0.48, 0.46, 0, 0.42, 0)]), color: CR, name: 'cabinet' },
    // キャビネット扉2枚
    { ...merge([box(0.28, 0.44, 0.012, -0.14, 0.42, 0.235)]), color: CR, name: 'cabinet_door_l' },
    { ...merge([box(0.28, 0.44, 0.012, 0.14, 0.42, 0.235)]), color: CR, name: 'cabinet_door_r' },
    // 扉取っ手
    { ...merge([box(0.015, 0.06, 0.015, -0.02, 0.45, 0.245), box(0.015, 0.06, 0.015, 0.02, 0.45, 0.245)]), color: MS, name: 'cabinet_handles', metallic: 0.7 },
    // キャビネット台座
    { ...merge([box(0.56, 0.06, 0.03, 0, 0.03, 0.23)]), color: WD, name: 'cabinet_plinth' },
  ],
  fridge: () => [
    // 本体
    { ...merge([box(0.65, 1.70, 0.65, 0, 0.85, 0)]), color: MS, name: 'body', metallic: 0.6 },
    // 上段ドア
    { ...merge([box(0.63, 0.46, 0.012, 0, 1.44, 0.335)]), color: [0.72, 0.72, 0.75], name: 'door_upper', metallic: 0.65 },
    // 下段ドア
    { ...merge([box(0.63, 0.96, 0.012, 0, 0.74, 0.335)]), color: [0.72, 0.72, 0.75], name: 'door_lower', metallic: 0.65 },
    // ドア分割ライン
    { ...merge([box(0.63, 0.008, 0.018, 0, 1.20, 0.33)]), color: [0.55, 0.55, 0.58], name: 'divider', metallic: 0.5 },
    // 上段取っ手
    { ...merge([box(0.015, 0.10, 0.022, 0.30, 1.44, 0.355)]), color: MD, name: 'handle_upper', metallic: 0.8 },
    // 下段取っ手
    { ...merge([box(0.015, 0.14, 0.022, 0.30, 1.00, 0.355)]), color: MD, name: 'handle_lower', metallic: 0.8 },
    // 底面通気口
    { ...merge([box(0.50, 0.03, 0.012, 0, 0.015, 0.33)]), color: [0.5, 0.5, 0.53], name: 'vent', metallic: 0.4 },
  ],
  refrigerator: () => [
    // 本体
    { ...merge([box(0.75, 1.80, 0.70, 0, 0.90, 0)]), color: MS, name: 'body', metallic: 0.6 },
    // 上段ドア（冷凍室）
    { ...merge([box(0.73, 0.52, 0.015, 0, 1.56, 0.355)]), color: [0.72, 0.72, 0.75], name: 'door_upper', metallic: 0.65 },
    // 下段ドア（冷蔵室）
    { ...merge([box(0.73, 1.06, 0.015, 0, 0.81, 0.355)]), color: [0.72, 0.72, 0.75], name: 'door_lower', metallic: 0.65 },
    // ドア分割ライン（横線）
    { ...merge([box(0.73, 0.008, 0.020, 0, 1.29, 0.355)]), color: [0.55, 0.55, 0.58], name: 'door_divider', metallic: 0.5 },
    // 上段取っ手（バーハンドル）
    { ...merge([box(0.018, 0.14, 0.025, 0.34, 1.56, 0.375)]), color: MD, name: 'handle_upper_bar', metallic: 0.85 },
    { ...merge([box(0.018, 0.015, 0.020, 0.34, 1.49, 0.365), box(0.018, 0.015, 0.020, 0.34, 1.63, 0.365)]), color: MD, name: 'handle_upper_mounts', metallic: 0.8 },
    // 下段取っ手（バーハンドル）
    { ...merge([box(0.018, 0.18, 0.025, 0.34, 1.05, 0.375)]), color: MD, name: 'handle_lower_bar', metallic: 0.85 },
    { ...merge([box(0.018, 0.015, 0.020, 0.34, 0.96, 0.365), box(0.018, 0.015, 0.020, 0.34, 1.14, 0.365)]), color: MD, name: 'handle_lower_mounts', metallic: 0.8 },
    // メーカーロゴプレート
    { ...merge([box(0.10, 0.02, 0.005, 0, 1.72, 0.355)]), color: [0.6, 0.6, 0.63], name: 'logo_plate', metallic: 0.7 },
    // 底面通気口
    { ...merge([box(0.60, 0.04, 0.015, 0, 0.02, 0.35)]), color: [0.5, 0.5, 0.53], name: 'vent', metallic: 0.4 },
    // 上面（微妙に凹み）
    { ...merge([box(0.70, 0.005, 0.65, 0, 1.802, 0)]), color: [0.66, 0.66, 0.69], name: 'top_plate', metallic: 0.5 },
  ],
  display_case: () => [
  ...
  tv_monitor: () => [
    // 画面（ベゼルの内側）
    { ...merge([box(0.74, 0.44, 0.005, 0, 0.66, 0.014)]), color: [0.05, 0.05, 0.08], name: 'screen_panel' },
    // ベゼル（薄いフレーム、上左右は細く下は少し太い）
    { ...merge([box(0.80, 0.50, 0.025, 0, 0.66, 0)]), color: MD, name: 'bezel', metallic: 0.3 },
    // 上ベゼル
    { ...merge([box(0.78, 0.015, 0.028, 0, 0.91, 0.002)]), color: [0.12, 0.12, 0.14], name: 'bezel_top' },
    // 下ベゼル（ロゴ付き、少し太い）
    { ...merge([box(0.78, 0.025, 0.028, 0, 0.415, 0.002)]), color: [0.12, 0.12, 0.14], name: 'bezel_bottom' },
    // メーカーロゴ
    { ...merge([box(0.04, 0.008, 0.005, 0, 0.42, 0.018)]), color: [0.35, 0.35, 0.38], name: 'logo', metallic: 0.5 },
    // 背面パネル
    { ...merge([box(0.70, 0.40, 0.015, 0, 0.66, -0.02)]), color: [0.15, 0.15, 0.15], name: 'back_panel' },
    // 背面通気口
    { ...merge([box(0.30, 0.06, 0.005, 0, 0.75, -0.028)]), color: [0.20, 0.20, 0.22], name: 'back_vent' },
    // スタンドネック
    { ...merge([box(0.06, 0.18, 0.06, 0, 0.30, -0.02)]), color: MD, name: 'stand_neck', metallic: 0.5 },
    // スタンドアーム
    { ...merge([box(0.04, 0.04, 0.10, 0, 0.40, -0.03)]), color: MD, name: 'stand_arm', metallic: 0.5 },
    // ベース台座
    { ...merge([box(0.30, 0.015, 0.20, 0, 0.008, 0)]), color: MD, name: 'base', metallic: 0.5 },
    // ベース前面エッジ
    { ...merge([cyl(0.008, 0.008, 0.30, 8, 0, 0.008, 0.10)]), color: MD, name: 'base_edge', metallic: 0.5 },
    // 電源LEDインジケーター
    { ...merge([box(0.006, 0.006, 0.005, 0.35, 0.42, 0.018)]), color: [0.2, 0.6, 0.9], name: 'power_led' },
  ],
  washing_machine: () => [
  ...
  air_conditioner: () => [
    ...
    // 上面パネル（吸気グリル表現: 横溝5本）
    { ...merge([box(0.70, 0.003, 0.04, 0, 0.28, -0.04), box(0.70, 0.003, 0.04, 0, 0.28, 0.00), box(0.70, 0.003, 0.04, 0, 0.28, 0.04)]), color: [0.88, 0.88, 0.88], name: 'intake_grilles' },
    // ルーバー（風向き板）5枚 — 各板の間に隙間
    { ...merge([box(0.74, 0.005, 0.025, 0, 0.02, 0.07)]), color: [0.92, 0.92, 0.92], name: 'louver_1' },
    { ...merge([box(0.74, 0.005, 0.025, 0, 0.01, 0.04)]), color: [0.92, 0.92, 0.92], name: 'louver_2' },
    { ...merge([box(0.74, 0.005, 0.025, 0, 0.00, 0.01)]), color: [0.92, 0.92, 0.92], name: 'louver_3' },
    { ...merge([box(0.74, 0.005, 0.025, 0, -0.01, -0.02)]), color: [0.90, 0.90, 0.90], name: 'louver_4' },
    { ...merge([box(0.74, 0.005, 0.025, 0, -0.02, -0.05)]), color: [0.90, 0.90, 0.90], name: 'louver_5' },
    // 吹出口フレーム
    { ...merge([box(0.76, 0.04, 0.005, 0, -0.01, 0.095), box(0.76, 0.04, 0.005, 0, -0.01, -0.07)]), color: [0.88, 0.88, 0.88], name: 'vent_frame' },
    // 前面パネル（右端に運転ランプ）
    { ...merge([box(0.78, 0.22, 0.005, 0, 0.14, 0.113)]), color: [0.96, 0.96, 0.96], name: 'front_panel' },
    // 運転ランプ（小さい緑の点）
    { ...merge([box(0.008, 0.008, 0.006, 0.35, 0.24, 0.116)]), color: [0.2, 0.8, 0.3], name: 'led_indicator' },
    // メーカーロゴプレート
    { ...merge([box(0.06, 0.012, 0.005, 0.30, 0.06, 0.116)]), color: [0.85, 0.85, 0.85], name: 'logo' },
    // 左右端のR表現
    { ...merge([cyl(0.025, 0.025, 0.28, 10, -0.40, 0.14, 0), cyl(0.025, 0.025, 0.28, 10, 0.40, 0.14, 0)]), color: [0.93, 0.93, 0.93], name: 'side_curves' },
  ],
  bar_table: () => [
  ...
  register_counter: () => [
    ...
    // 天板前面ノーズ
    { ...merge([cyl(0.006, 0.006, 1.24, 8, 0, 0.948, 0.32)]), color: CR, name: 'top_nose' },
    // ボディ前面+側面+背面
    { ...merge([box(1.16, 0.56, 0.028, 0, 0.62, 0.29)]), color: W, name: 'front' },
    { ...merge([box(0.028, 0.56, 0.56, -0.58, 0.62, 0), box(0.028, 0.56, 0.56, 0.58, 0.62, 0)]), color: W, name: 'sides' },
    { ...merge([box(1.16, 0.56, 0.018, 0, 0.62, -0.29)]), color: WD, name: 'back' },
    // 内部棚板
    { ...merge([box(1.12, 0.018, 0.52, 0, 0.50, 0)]), color: WL, name: 'shelf' },
    // 台座（巾木）
    { ...merge([box(1.18, 0.05, 0.025, 0, 0.025, 0.29)]), color: MD, name: 'plinth' },
    // タブレット画面（画面+ベゼル）
    { ...merge([box(0.26, 0.16, 0.005, 0.32, 1.08, 0.057)]), color: [0.05, 0.05, 0.08], name: 'tablet_screen' },
    { ...merge([box(0.28, 0.18, 0.012, 0.32, 1.08, 0.05)]), color: [0.10, 0.10, 0.12], name: 'tablet_bezel' },
    // タブレットホームボタン
    { ...merge([cyl(0.008, 0.008, 0.005, 8, 0.32, 0.98, 0.058)]), color: [0.15, 0.15, 0.18], name: 'tablet_home_btn' },
    // タブレットスタンド
    { ...merge([cyl(0.012, 0.018, 0.12, 8, 0.32, 0.98, 0.05)]), color: MD, name: 'screen_stand', metallic: 0.5 },
    { ...merge([box(0.08, 0.008, 0.08, 0.32, 0.94, 0.05)]), color: MD, name: 'screen_base', metallic: 0.5 },
    // カードリーダー本体
    { ...merge([box(0.07, 0.018, 0.13, -0.30, 0.945, 0.12)]), color: [0.12, 0.12, 0.12], name: 'card_reader' },
    // カードリーダー画面
    { ...merge([box(0.05, 0.005, 0.06, -0.30, 0.958, 0.10)]), color: [0.08, 0.12, 0.20], name: 'card_reader_screen' },
    // カードリーダー挿入口
    { ...merge([box(0.05, 0.004, 0.005, -0.30, 0.942, 0.185)]), color: [0.08, 0.08, 0.08], name: 'card_slot' },
    // タップ決済マーク（NFCシンボル風の小プレート）
    { ...merge([box(0.02, 0.02, 0.005, -0.30, 0.96, 0.15)]), color: [0.2, 0.5, 0.8], name: 'nfc_icon' },
    // レシートプリンター
    { ...merge([box(0.12, 0.10, 0.14, -0.10, 0.94, -0.10)]), color: [0.18, 0.18, 0.20], name: 'receipt_printer' },
    // レシート排出口
    { ...merge([box(0.08, 0.004, 0.008, -0.10, 0.99, -0.03)]), color: [0.25, 0.25, 0.28], name: 'receipt_slot' },
  ],
  menu_board: () => [
  ...
  rug: () => [
    { ...merge([box(1.50, 0.01, 1.00, 0, 0.005, 0)]), color: [0.72, 0.62, 0.48], name: 'rug', roughness: 0.85 },
  ],
  curtain: () => [
  ...
  toilet: () => {
    const WH = [0.95, 0.95, 0.95]; // white ceramic
    const WH2 = [0.92, 0.92, 0.93]; // slightly darker
    return [
      // ボウル下部（台座、楕円形）
      { ...merge([cyl(0.17, 0.20, 0.10, 14, 0, 0.05, 0.04)]), color: WH, name: 'pedestal' },
      // ボウル本体（前方に伸びた楕円）
      { ...merge([cyl(0.18, 0.16, 0.22, 14, 0, 0.22, 0.04)]), color: WH, name: 'bowl_outer' },
      // ボウル前方の延長部分
      { ...merge([box(0.30, 0.20, 0.12, 0, 0.22, 0.16)]), color: WH, name: 'bowl_front' },
      // ボウル前面の丸み
      { ...merge([cyl(0.15, 0.15, 0.20, 14, 0, 0.22, 0.22)]), color: WH, name: 'bowl_nose' },
      // ボウル内部（凹み表現）
      { ...merge([cyl(0.14, 0.12, 0.10, 12, 0, 0.28, 0.06)]), color: WH2, name: 'bowl_inner' },
      // 便座リング
      { ...merge([cyl(0.19, 0.19, 0.018, 14, 0, 0.345, 0.06)]), color: WH, name: 'seat_ring_outer' },
      { ...merge([cyl(0.14, 0.14, 0.020, 12, 0, 0.345, 0.06)]), color: WH2, name: 'seat_ring_hole' },
      // 便座前方
      { ...merge([box(0.30, 0.018, 0.10, 0, 0.345, 0.18)]), color: WH, name: 'seat_front' },
      // 便蓋（閉じた状態）
      { ...merge([cyl(0.19, 0.19, 0.012, 14, 0, 0.36, 0.06)]), color: WH, name: 'lid_round' },
      { ...merge([box(0.30, 0.012, 0.10, 0, 0.36, 0.18)]), color: WH, name: 'lid_front' },
      // タンク本体
      { ...merge([box(0.36, 0.38, 0.16, 0, 0.54, -0.14)]), color: WH, name: 'tank' },
      // タンク蓋
      { ...merge([box(0.38, 0.02, 0.18, 0, 0.74, -0.14)]), color: WH, name: 'tank_lid' },
      // レバー（タンク側面）
      { ...merge([cyl(0.008, 0.008, 0.06, 8, 0.20, 0.70, -0.06)]), color: MS, name: 'flush_lever', metallic: 0.8 },
      // ヒンジ（便座とタンクの接続部）
      { ...merge([cyl(0.012, 0.012, 0.06, 8, -0.08, 0.35, -0.06), cyl(0.012, 0.012, 0.06, 8, 0.08, 0.35, -0.06)]), color: MS, name: 'hinges', metallic: 0.7 },
    ];
  },
  armchair: () => [
    // 座面フレーム
  ...
  washbasin: () => [
    // カウンタートップ
    { ...merge([box(0.56, 0.03, 0.44, 0, 0.81, 0)]), color: [0.95, 0.95, 0.95], name: 'top' },
    // 洗面ボウル（段差で深さ表現）
    { ...merge([cyl(0.18, 0.18, 0.01, 14, 0, 0.80, 0.02)]), color: [0.93, 0.93, 0.95], name: 'bowl_rim' },
    { ...merge([cyl(0.17, 0.15, 0.04, 14, 0, 0.775, 0.02)]), color: [0.94, 0.94, 0.96], name: 'bowl_mid' },
    { ...merge([cyl(0.14, 0.12, 0.04, 12, 0, 0.74, 0.02)]), color: [0.95, 0.95, 0.97], name: 'bowl_deep' },
    // 排水口
    { ...merge([cyl(0.015, 0.015, 0.008, 8, 0, 0.72, 0.02)]), color: MS, name: 'drain', metallic: 0.8 },
    // 蛇口ベース
    { ...merge([cyl(0.020, 0.025, 0.035, 12, 0, 0.845, -0.15)]), color: MS, name: 'faucet_base', metallic: 0.85 },
    // 蛇口アーム（垂直）
    { ...merge([cyl(0.010, 0.010, 0.14, 10, 0, 0.93, -0.15)]), color: MS, name: 'faucet_stem', metallic: 0.85 },
    // 蛇口スパウト（前方へ）
    { ...merge([cyl(0.008, 0.008, 0.12, 8, 0, 1.00, -0.08)]), color: MS, name: 'faucet_spout', metallic: 0.85 },
    // 蛇口先端
    { ...merge([cyl(0.006, 0.010, 0.025, 8, 0, 0.985, -0.02)]), color: MS, name: 'faucet_tip', metallic: 0.85 },
    // レバーハンドル
    { ...merge([box(0.05, 0.008, 0.015, 0, 0.96, -0.15)]), color: MS, name: 'faucet_lever', metallic: 0.85 },
    // キャビネット本体
    { ...merge([box(0.52, 0.48, 0.40, 0, 0.54, 0)]), color: CR, name: 'cabinet' },
    // キャビネット扉
    { ...merge([box(0.24, 0.44, 0.010, -0.13, 0.52, 0.21)]), color: CR, name: 'cabinet_door_l' },
    { ...merge([box(0.24, 0.44, 0.010, 0.13, 0.52, 0.21)]), color: CR, name: 'cabinet_door_r' },
    // 扉取っ手
    { ...merge([box(0.012, 0.05, 0.012, -0.02, 0.55, 0.22), box(0.012, 0.05, 0.012, 0.02, 0.55, 0.22)]), color: MS, name: 'cabinet_handles', metallic: 0.7 },
    // 鏡（壁掛け）
    { ...merge([box(0.42, 0.55, 0.008, 0, 1.18, -0.22)]), color: [0.85, 0.88, 0.92], name: 'mirror', metallic: 0.9 },
    // 鏡フレーム
    { ...merge([box(0.46, 0.59, 0.006, 0, 1.18, -0.225)]), color: MD, name: 'mirror_frame', metallic: 0.4 },
    // 台座（巾木）
    { ...merge([box(0.50, 0.05, 0.025, 0, 0.025, 0.20)]), color: WD, name: 'plinth' },
  ],
  stairs: () => [
  ...
  coffee_machine: () => [
    ...
    // 上部水タンク
    { ...merge([box(0.26, 0.10, 0.12, 0, 0.40, -0.10)]), color: [0.3, 0.3, 0.35], name: 'water_tank', metallic: 0.3 },
    // 操作パネル（ボタン付き）
    { ...merge([box(0.24, 0.06, 0.008, 0, 0.30, 0.18)]), color: [0.15, 0.15, 0.18], name: 'panel' },
    // ボタン3個（エスプレッソ/ラテ/電源）
    { ...merge([cyl(0.012, 0.012, 0.008, 8, -0.06, 0.30, 0.188), cyl(0.012, 0.012, 0.008, 8, 0, 0.30, 0.188), cyl(0.012, 0.012, 0.008, 8, 0.06, 0.30, 0.188)]), color: [0.4, 0.4, 0.45], name: 'buttons', metallic: 0.5 },
    // 小型ディスプレイ
    { ...merge([box(0.08, 0.03, 0.005, 0, 0.34, 0.185)]), color: [0.15, 0.25, 0.45], name: 'display' },
    // 抽出口（ノズル2本）
    { ...merge([cyl(0.008, 0.010, 0.06, 8, -0.02, 0.15, 0.12), cyl(0.008, 0.010, 0.06, 8, 0.02, 0.15, 0.12)]), color: [0.20, 0.20, 0.22], name: 'nozzles', metallic: 0.6 },
    // 抽出口カバー
    { ...merge([box(0.10, 0.03, 0.04, 0, 0.20, 0.12)]), color: MD, name: 'nozzle_housing', metallic: 0.5 },
    // カップ受け台（グリッド表現）
    { ...merge([box(0.14, 0.008, 0.12, 0, 0.08, 0.14)]), color: MS, name: 'drip_tray_base', metallic: 0.6 },
    // グリッド（横3本）
    { ...merge([box(0.12, 0.004, 0.005, 0, 0.088, 0.10), box(0.12, 0.004, 0.005, 0, 0.088, 0.14), box(0.12, 0.004, 0.005, 0, 0.088, 0.18)]), color: MS, name: 'drip_grid', metallic: 0.7 },
    // カップウォーマー（上面）
    { ...merge([box(0.26, 0.005, 0.15, 0, 0.355, 0.04)]), color: [0.22, 0.22, 0.25], name: 'cup_warmer', metallic: 0.4 },
    // 豆ホッパー蓋
    { ...merge([cyl(0.04, 0.04, 0.02, 10, 0.08, 0.465, -0.08)]), color: [0.2, 0.2, 0.22], name: 'hopper_lid', metallic: 0.3 },
  ],
  // ─── オフィス向け ───
  office_desk: () => [
    // 天板（面取りエッジ）
  ...
  vending_machine: () => [
    ...
    // 商品陳列窓（ガラス面）
    { ...merge([box(0.55, 1.06, 0.015, 0, 1.12, 0.34)]), color: [0.20, 0.40, 0.80], name: 'display_glass' },
    // 陳列窓フレーム
    { ...merge([box(0.57, 0.015, 0.020, 0, 1.65, 0.34), box(0.57, 0.015, 0.020, 0, 0.59, 0.34)]), color: [0.80, 0.80, 0.82], name: 'display_frame_tb' },
    { ...merge([box(0.015, 1.06, 0.020, -0.28, 1.12, 0.34), box(0.015, 1.06, 0.020, 0.28, 1.12, 0.34)]), color: [0.80, 0.80, 0.82], name: 'display_frame_lr' },
    // 商品スロット横仕切り（3列）
    { ...merge([box(0.005, 1.04, 0.010, -0.09, 1.12, 0.35), box(0.005, 1.04, 0.010, 0.09, 1.12, 0.35)]), color: [0.70, 0.70, 0.73], name: 'slot_dividers_v' },
    // 商品スロット棚（5段）
    { ...merge([box(0.53, 0.005, 0.010, 0, 0.70, 0.35), box(0.53, 0.005, 0.010, 0, 0.90, 0.35), box(0.53, 0.005, 0.010, 0, 1.10, 0.35), box(0.53, 0.005, 0.010, 0, 1.30, 0.35), box(0.53, 0.005, 0.010, 0, 1.50, 0.35)]), color: [0.70, 0.70, 0.73], name: 'slot_shelves' },
    // 商品サンプル（棚上の小さなbox群）
    { ...merge([
      box(0.04, 0.10, 0.04, -0.18, 0.76, 0.34), box(0.04, 0.10, 0.04, 0, 0.76, 0.34), box(0.04, 0.10, 0.04, 0.18, 0.76, 0.34),
      box(0.04, 0.10, 0.04, -0.18, 0.96, 0.34), box(0.04, 0.10, 0.04, 0, 0.96, 0.34), box(0.04, 0.10, 0.04, 0.18, 0.96, 0.34),
      box(0.04, 0.10, 0.04, -0.18, 1.16, 0.34), box(0.04, 0.10, 0.04, 0, 1.16, 0.34), box(0.04, 0.10, 0.04, 0.18, 1.16, 0.34),
    ]), color: [0.3, 0.6, 0.3], name: 'product_samples' },
    // 取出口（下部の凹み）
    { ...merge([box(0.48, 0.18, 0.035, 0, 0.28, 0.325)]), color: [0.18, 0.18, 0.20], name: 'dispenser_cavity' },
    // 取出口フラップ（半透明プレート）
    { ...merge([box(0.46, 0.02, 0.030, 0, 0.38, 0.330)]), color: [0.6, 0.6, 0.62], name: 'dispenser_flap', metallic: 0.3 },
    // コイン投入口
    { ...merge([box(0.06, 0.04, 0.015, 0.24, 0.85, 0.35)]), color: MS, name: 'coin_panel', metallic: 0.6 },
    { ...merge([box(0.03, 0.003, 0.010, 0.24, 0.86, 0.36)]), color: [0.3, 0.3, 0.32], name: 'coin_slit' },
    // 紙幣投入口
    { ...merge([box(0.08, 0.02, 0.015, 0.24, 0.78, 0.35)]), color: [0.4, 0.4, 0.42], name: 'bill_slot' },
    // ボタン（商品選択、右側に3x2）
    { ...merge([
      cyl(0.012, 0.012, 0.01, 8, 0.24, 1.30, 0.355), cyl(0.012, 0.012, 0.01, 8, 0.24, 1.20, 0.355), cyl(0.012, 0.012, 0.01, 8, 0.24, 1.10, 0.355),
      cyl(0.012, 0.012, 0.01, 8, 0.24, 1.00, 0.355),
    ]), color: [0.8, 0.8, 0.2], name: 'select_buttons' },
    // 返却レバー
    { ...merge([cyl(0.015, 0.015, 0.02, 8, 0.24, 0.65, 0.35)]), color: MS, name: 'return_lever', metallic: 0.7 },
    // おつり取出口
    { ...merge([box(0.08, 0.03, 0.020, 0.24, 0.55, 0.34)]), color: MS, name: 'change_tray', metallic: 0.5 },
    // 上部看板照明帯
    { ...merge([box(0.60, 0.03, 0.010, 0, 1.72, 0.34)]), color: [0.9, 0.85, 0.5], name: 'header_light' },
  ],
  atm: () => [
